...
async def log_command(ctx: commands.Context):
	logger.info('invoking "%s"', ctx.message)


async def handle_command_error(ctx: commands.Context, err: Exception):
	if isinstance(err, commands.CommandError):
		if isinstance(err, commands.errors.MissingRequiredArgument):
			await ctx.reply(f'Parameter <{err.param}> required.')
		else:
			await ctx.reply(str(err))
	else:
		await ctx.reply(':poop: ERROR :poop:')
		raise err
